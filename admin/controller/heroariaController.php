...
require_once('./model/heroariaModel.php');
require_once('./controller/UploadFile.php');
class heroaria extends Controller
{

    public $active = 'heroaria'; //for highlighting the active link...
    private $Model;
    ...
    public function createHero($data, $file)
    {
        ...
        $Response = $this->Model->createheroaria($Payload);
    ...
    {
        return $this->Model->editheroaria($id);
    }
    ...
    public function Update(array $data, $file)
    {
        ...
        $Response = $this->Model->Updateheroaria($Payload);
    ...
    public function delete($id)
    {
        ...
        $response = $this->Model->deleteheroaria($id);
        if (!$response) {
            $Response['status'] = 'Sorry, An unexpected error occurred and your request could not be completed.';
            header("location:HeroIndex.php");
            return $Response;
